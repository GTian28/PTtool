...
在dirlink.sh的基础上有两点改变：
- 递归到每一个有文件的文件夹去创建硬链接
- 硬链接的文件可以随意移动，当某个文件夹下的所有文件都没有硬链接（比如在emby、jellyfin等平台上删除了不想看的影片）后，删除源文件夹。之所以没有直接删除没有硬链接的文件，一个是因为小于1M的文件本身就没有创建硬链接，另一个是因为防止删除单一文件后影响整体保种，所以当整个文件夹下的文件都没有硬链接后，应该是说明这一整个资源都没用了，所以才要删除源文件。

但因为会递归到每一个文件，所以耗费时间较长，并且这个代码会删除源文件，所以小心使用。
...
数据无价，小心操作。
本脚本（除autolink.sh、ddirlink.sh外）没有rm删除，只有mkdir和cp， 最多搞乱文件系统。但要注意不要把目的地目录设置到系统目录去了。
一切后果自负
